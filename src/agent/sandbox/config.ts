...
export type SandboxConfig = {
  image: string; datasetsPath: string; cpus: number; memoryMb: number
...
export function normalizeSandboxWorkdir(input?: string) {
  ...
  if (absolute !== "/workspace" && !absolute.startsWith("/workspace/") && absolute !== "/datasets" && !absolute.startsWith("/datasets/")) {
    throw new Error("workdir must be beneath /workspace or /datasets.")
  }
  return absolute
}
